...
import CardComponentCarouselInCard from '../../../../CardComponent/components/CardComponentCarouselInCard'
import placeholder_image from '../../../../../assets/img/placeholder_image.png'

class TransactionCardDetails extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            categorie: null,
    ...
    //     var query = transactionRef.where("id", "==", id);
    ...
    //     const db = firebase.firestore();
    //     // const transactionDocs = await db.collection('transactions').where('contributeur', '==', this.props.user.email).get();
    //     const transactionDocs = await db.collection('transactions').get();
    //     transactionDocs.docs.forEach(doc => transactions.push(doc.data()));

    //     transactions.forEach(est => {
    //         if(!est.isDeleted) {
    //             estimationDispatchDatas.push(est)
    //         }
    //     })
    //     this.props.dispatch({ type: 'SET_MY_TRANSACTIONS', data: estimationDispatchDatas.filter(trans => trans.contributeur === this.props.email)});
    // }
    componentDidMount () {
        // this.getUsser()
    }
    ...
    render() {
        ...
        //   };
        return (
            <div className = "TransactionCardDetails">
                ...
                <div className = "transactionTitle">
                    ...
                    {this.props.espacePro.TransactionDetails.consistance} de {this.props.espacePro.TransactionDetails.surface} m²
                    </p>
                    <p className="details-bien">Vendu en {this.props.espacePro.TransactionDetails.dateTransactions}</p>
                </div>

                <hr></hr>
                <div className="details-bien-container-image carousel-transaction--container">
                                        { this.props.espacePro.TransactionDetails && this.props.espacePro.TransactionDetails.images && this.props.espacePro.TransactionDetails.images.length > 0 ? (
                    ...
                    (
                        <div className = "row">
                            ...
                            <div className = "col-md-6">
                                ...
                                (<p className="details-bien"><span className ="details-bien-title"> Résidence fermée : </span> {this.props.espacePro.TransactionDetails.residencefermee === 1 ? 'Oui' : 'Non'}</p>)
                                : '' }
                                ...
                                (<p className="details-bien"><span className ="details-bien-title"> Sans vis-à-vis : </span> {this.props.espacePro.TransactionDetails.vueexceptionnelle === 1 ? 'Oui' : 'Non'}</p>)
                                : '' }
                            </div>
                        </div>
                    ) 
...
{/* 
                ...
                {/* <div className = "transactionActions">
                        <h5>Actions</h5>
                        <button 
                        onClick={() => this.deleteTransaction(this.props.espacePro.TransactionDetails.id)}
                        type="button" 
                        className="button button-danger">Supprimer</button>
                </div>
                 */}
...
export default connect(mapStateToProps)(TransactionCardDetails);