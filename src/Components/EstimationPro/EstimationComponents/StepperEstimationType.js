...
class StepperEstimationType extends React.Component {
    ...
    returnPreviousStep = () => {
        this.props.dispatch({ type: 'PRO_SET_ACTIVE_STEP',data: 0 });
    }
        ...
        return <div className="row justify-content">
            <div className="col-md-12 col-sm-12 col-lg-8">
                <div className="step second-step">
                    ...
                    onClose={() =>{ this.props.dispatch({ type: 'PRO_UPDATE_ACTIVE_HOUSE', data: 2 }) }}
                    aria-labelledby="alert-dialog-title"
                    aria-describedby="alert-dialog-description">
                        <DialogContent>
                            <DialogContentText id="alert-dialog-estimate">
                                <div className = "close" onClick = {() =>{ this.props.dispatch({ type: 'PRO_UPDATE_ACTIVE_HOUSE', data: 2 }) }}> x </div>
                            ...
                            return(
                                <div className="col-sm-12 col-md-4" key={index} >
                                    <div className={`estimation-type estimation-type-${type.id}`} onClick={() =>{ 
                                         if (type.id === 1) {
                                            this.props.dispatch({ type: 'PRO_ESTIMATION_TYPE', data: 'villa' });
                                            this.onHandleNextChange()
                                            // this.props.dispatch({ type: 'PRO_UPDATE_ACTIVE_HOUSE', data: type.id }) 

                                         }else if(type.id === 2) {
                                            this.props.dispatch({ type: 'PRO_ESTIMATION_TYPE', data: 'appartement' });
                                            this.onHandleNextChange()
                                         }
                                         else {
                                            this.props.dispatch({ type: 'PRO_UPDATE_ACTIVE_HOUSE', data: type.id }) }
...
export default connect(mapStateToProps)(StepperEstimationType);