...
create_mapbox_places_url(){
    ...
    src=src+`/${lng},${lat},14.14,0/826x450@2x?before_layer=admin-1-boundary-bg&access_token=pk.eyJ1IjoiYmFkcmJlbGtleml6IiwiYSI6ImNraDduNjN2bDA2MGszMG5zZHRqNm5zMzIifQ.oITjlONmSiUQsFKrZfFd3w`
    // console.log(src)
...
// datasets: [
// {
// label: 'Commerces à moins de 500 mètres de votre bien',
...
{/* <Dialog
                        maxWidth={"md"}
                        open={this.state.loadingPDF}
                        disableBackdropClick={true}
                        aria-labelledby="max-width-dialog-title"
                    >
                        <DialogTitle id="max-width-dialog-title">Création du rapport PDF</DialogTitle>
                        <DialogContent>
...
<p className="report-synthesis">Votre villa située {this.state.rapport.addresseForcee && this.state.rapport.addresseForcee!=="" ?  (this.state.rapport.addresseForcee) :  (this.state.estimation.adresse)} d'une surface construite de {parseInt(this.state.estimation.surfaceconstruite)} m<sup>2</sup> sur un terrain de {parseInt(this.state.estimation.surfaceterrain)} m<sup>2</sup> </p>
)}
{/* {this.state.estimation.bien==="appartement" ? 
(
<>
...
<div className="synthese--item-container"> 
<AirlineSeatIndividualSuiteIcon /> <p className="synthese--item">{this.formatTypologieItem(this.state.estimation.typologie)}</p>
</div>

<div className="synthese--item-container"> 
<EditLocationIcon /> <p className="synthese--item-adresse">{this.formatAdresse(this.state.estimation.adresse)}</p>
</div>   
</div>
</>                                                                         
) 
...
<div className="synthese--item-container"> 
<SquareFootIcon /> <p className="synthese--item">{this.state.estimation.surfaceterrain} m<sup>2</sup></p>
</div>
<div className="synthese--item-container"> 
<AirlineSeatIndividualSuiteIcon /> <p className="synthese--item">{this.formatTypologieItem(this.state.estimation.typologie)}</p>
</div>
...
<div className="synthese--item-container"> 
<EditLocationIcon /> <p className="synthese--item synthese--item-adresse">{this.formatAdresse(this.state.estimation.adresse)}</p>
</div> 
</div>
</>
)} */}
...
<CardContent>
<p className="zone--listing">Votre bien se situe dans la zone <span className="zone">{this.state.estimation.zone}</span> du référentiel de l'ANCFCC</p>
...
<div className="map2">
<Grid item xs={12}>
<div className="mapbox--container" id ="mapbox--container">
      ...
      zoom={14}>
        ...
        <Marker 
               position={{lat: parseFloat(`${this.state.rapport.localisation.split("lat : ")[1].split(" ")[0]}`), lng: parseFloat(`${this.state.rapport.localisation.split("lng : ")[1]}`)}}/>
...
<ServiceMap viewport={{latitude : parseFloat(`${this.state.rapport.localisation.split("lat : ")[1].split(" ")[0]}`), longitude : parseFloat(`${this.state.rapport.localisation.split("lng : ")[1]}`), zoom : 14}} localisation={this.state.rapport.localisation} list_restaurants={this.state.rapport.list_restaurants} list_ecoles={this.state.rapport.list_ecoles} list_pharmacies={this.state.rapport.list_pharmacies}
list_supermarches={this.state.rapport.list_supermarches}/><Card variant="outlined">
<CardContent>
...
<p className="radius-subtitle">Les biens comparables dans un rayon de 500 mètres autour de {this.state.estimation.bien==="appartement" ? 'votre appartement' : 'votre villa'} </p>
</div>
...
pharmaCount={this.state.rapport.places_found.filter(item => item.type === 'Pharmacie').length} restauCount={this.state.rapport.places_found.filter(item => item.type === 'Restaurant').length} superMCount={this.state.rapport.places_found.filter(item => item.type === 'Supermarché').length} autreCount={this.state.rapport.places_found.filter(item => item.type === '').length} lng={parseFloat(`${this.state.rapport.localisation.split("lng : ")[1]}`)} lat={parseFloat(`${this.state.rapport.localisation.split("lat : ")[1].split(" ")[0]}`)} rapport={this.state.rapport} estimation={this.state.estimation} agence={this.state.agence}  syntheseUrlFirstPage={this.state.syntheseUrlFirstPage} syntheseUrlThirdPage={this.state.syntheseUrlThirdPage} syntheseUrlSecondPage={this.state.syntheseUrlSecondPage} syntheseUrlFourthPage={this.state.syntheseUrlFourthPage} syntheseUrlFifthPage={this.state.syntheseUrlFifthPage} date={moment(this.state.estimation.date).format('LL')}/>} fileName={`Rapport d'estimation ${this.state.agence.nameEntreprise} ${this.state.rapport.reference}.pdf`}>
...
<Image src={`https://api.mapbox.com/styles/v1/mapbox/streets-v11/static/pin-l+40b1ce(${props.lng},${props.lat})/${props.lng},${props.lat},15.14,0/826x450@2x?before_layer=admin-1-boundary-bg&access_token=pk.eyJ1IjoiYmFkcmJlbGtleml6IiwiYSI6ImNraDduNjN2bDA2MGszMG5zZHRqNm5zMzIifQ.oITjlONmSiUQsFKrZfFd3w`}
...
<Text debug={false} style={styles.PhotoTitle} > Évolution du prix de votre bien </Text>
...
{ props.rapport.annonces_selected.length >0 || props.rapport.transactions_selected.length >0 ? (
    ...
    <Text debug={false} style={styles.PhotoSubTitle} > Les biens comparables dans un rayon de 500 mètres autour de {props.estimation.bien==="appartement" ? 'votre appartement' : 'votre villa'} </Text>
    </View>
...
<Text debug={false} style={styles.listingTitle} >
    Vente d'un(e) {row.consistance} d'une surface de {row.surface} m² </Text>
    ...
    {formatConstruction(row.construction) !=="" ? (
        <Text debug={false} style={styles.listingDate} >Construction : {formatConstruction(row.construction)}  </Text> )
 : (<Text></Text>)}    
...
{ row.images && row.images.length > 0 ? (
    ...
    {props.rapport.annonces_selected.map((row, index) => (
        ...
        <Text debug={false} style={styles.listingTitle} >Annonce pour {props.estimation.bien==="appartement" ? 'un appartement' : 'une villa'} de {row.chambres} chambre pour une surface de {row.surface} m²</Text>
<Text debug={false} style={styles.listingPrice} >Prix :  {new Intl.NumberFormat('us-US', { style: 'decimal', maximumFractionDigits : 0 }).format(row.prix).replace(/,/g," ")} MAD</Text>
<Text debug={false} style={styles.listingPriceM2} >{new Intl.NumberFormat('us-US', { style: 'decimal', maximumFractionDigits : 0 }).format(row.prix/row.surface).replace(/,/g," ")} MAD/m² </Text>
