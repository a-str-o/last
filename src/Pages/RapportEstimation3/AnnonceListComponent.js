...
render(){
    const { allAnnonces, currentAnnonces, currentPage, totalPages } = this.state;
    const totalAnnonces = allAnnonces.length;
    return (
<>
{this.state.currentAnnonces.map((row, index) => (
<AnnonceComponent bien={this.props.bien} chambres={row.chambres} prix={row.prix}  date_annonce={row.date_annonce} surface={row.surface} source={row.source} quartier={row.quartier} indice={index+1} />
))
}
