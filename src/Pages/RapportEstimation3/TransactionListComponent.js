...
{this.state.currentTransactions.map((row, index) => (
    <>
<TransactionComponent construction = {row.bien==="appartement" ? row.construction : row.anneeconstruction} address={row.address} transactionId = {row.transactionId} consistance={row.consistance} prix={row.prix} images={row.images} dateTransactions={row.dateTransactions} surface={row.surfaceeffective} indice={index+1} />
</>
))
}
